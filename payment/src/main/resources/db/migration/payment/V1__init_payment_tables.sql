...
(
    ...
    status              VARCHAR(20) NOT NULL COMMENT '결제 상태 REQUESTED/IN_PROGRESS/COMPLETED/FAILED/REFUNDED',
    failure_reason      VARCHAR(50) NULL COMMENT '결제 실패 사유',
    pg_tid              VARCHAR(36)    NULL COMMENT 'PG사 결제 ID(tid)',
    pg_requested_at     DATETIME(6)    NULL COMMENT 'pg 결제 요청 시각',
    created_at          DATETIME(6)    NOT NULL COMMENT '생성일',
    updated_at          DATETIME(6)    NOT NULL COMMENT '수정일',
    PRIMARY KEY (id),
    UNIQUE KEY uk_payment_order_number (order_number),
