...
- **포함**: 통합 이벤트 계약(`OrderCreated`, `PaymentCompleted`, `StockShortage` … payload record), 공통 메타 봉투 `EventEnvelope<T>`(`eventId`·`occurredAt`·`orderId`·`eventType` — 정책 PI-4), 토픽 이름 상수(이벤트당 1토픽, `MSG-<EVENT-NAME>`).
